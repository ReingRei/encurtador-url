...
export class GerenciadorDeSenhaService {
  ...
  async compararSenhas(senhaFornecida: string, hashArmazenado: string): Promise<boolean> {
    try {
      return await bcrypt.compare(senhaFornecida, hashArmazenado);
    } catch (error) {
      this.logger.error('Falha ao comparar senhas', error.stack);
      throw new InternalServerErrorException('Erro interno ao verificar a senha.');
    }
  }
}